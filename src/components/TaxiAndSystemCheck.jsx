      ...
      <div className="container px-5">
        ...
        <div className={`grid grid-cols-1 gap-6 md:grid-cols-3 ${location.pathname === "/brokerage" ? "xl:grid-cols-4" : "xl:grid-cols-5"}`}>
          {cards.map((card, idx) => {
            const CardContent = (
              <article
                className={`group relative h-full overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-900/70 to-slate-900/30 p-5 shadow-lg transition duration-300 hover:-translate-y-0.5 hover:shadow-2xl ${
                  card.link ? "cursor-pointer hover:border-slate-600" : ""
                }`}
              >
                {/* top gradient bar */}
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div className={`mx-auto md:mx-0 grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br ${card.gradient} text-slate-900`}>
                    {card.icon}
                  </div>
                </div>
            ...
            return card.link ? (
              <Link to={card.link} key={idx} className="block h-full">
                {CardContent}
              </Link>
            ) : (
